import {
    ...
    Bell, Shield, SlidersHorizontal, Heart,
    BookOpen, Star, Building2, Briefcase,
    GraduationCap, Sparkles, Edit3,
    Mail, Calendar,
} from "lucide-react";
...
const Profile = () => {
    ...
    const occupationIcon = user.tenantProfile?.occupationStatus === "student"
    ...
    return (
        <div className="flex flex-col gap-6 pb-10">
            ...
            <div className="rounded-2xl border border-white/5 bg-white/3 p-5 flex flex-col items-center gap-4 text-center relative">
                ...
                <div>
                    <div className="flex items-center justify-center gap-2">
                        ...
                        {user.verified && <CheckCircle2 size={15} className="text-green-400" />}
                    </div>


                    <div className="flex items-center justify-center gap-2 mt-1.5 flex-wrap">
                        <span className={`rounded-full border px-3 py-0.5 text-[11px] font-semibold ${isOwner
                            ? "border-orange-500/30 bg-orange-500/10 text-orange-300"
                            : "border-blue-500/30 bg-blue-500/10 text-blue-300"
                            }`}>
                            {user.role}
                        </span>
                        {age && <span className="text-[12px] text-zinc-500">{age} years old</span>}
            ...
            <div className="grid grid-cols-3 gap-3">
                {isTenant && (
                    <>
                        <StatCard label="Saved" value={0} icon={<Heart size={16} />} />
                        <StatCard label="Reviews" value={0} icon={<Star size={16} />} />
                    </>
                )}
                {isOwner && (
                    <>
                        <StatCard label="Properties" value={user.ownerProfile?.propertiesCount ?? 0} icon={<Building2 size={16} />} />
                        <StatCard label="Reviews" value={0} icon={<Star size={16} />} />
                    </>
                )}
            </div>
            ...
            <div className="flex flex-col gap-2">
                ...
                {isTenant && (
                    <>
                        <MenuItem icon={<Heart size={16} />} label="Saved Properties" sub="Your saved listings" onClick={() => navigate("/home/saved")} />
                        <MenuItem icon={<BookOpen size={16} />} label="My Bookings" sub="View booking history" onClick={() => navigate("/home/bookings")} />
                    </>
                )}
                <MenuItem icon={<Bell size={16} />} label="Notifications" sub="Manage your alerts" onClick={() => navigate("/home/notifications")} />
            </div>
